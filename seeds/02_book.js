...
exports.seed = function(knex, Promise) {
	return knex('genre').select().then(function(genre) {
		return Promise.all([
			knex('book').insert({
				title: 'Python In A Nutshell',
				genre_id: find.findFromList('Python', genre, 'name'),
			...
			knex('book').insert({
				...
				genre_id: find.findFromList('JavaScript', genre, 'name'),
			...
			knex('book').insert({
				title: 'Daniel\s Diary',
				genre_id: find.findFromList('Fantasy', genre, 'name'),
				description: 'The innermost thoughts of a strange computer nerd.',
				cover_url: 'http://runhaven.com/wp-content/uploads/Dear_Diary___by_Jeanutti-e1419202982799.jpg'
			})
		]);
	});
};
